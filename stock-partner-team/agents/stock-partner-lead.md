...
- `name: "redcard-challenger", subagent_type: "redcard-challenger"` ⚠️ 元流程角色，**不在第4步并行调度**，在第5步汇编完成后单独调用（详见「红牌挑战流程」）
...
| 🟥 | `redcard-challenger` | 赫问真 | 红牌挑战者 | 元流程角色——圆桌汇编后进场，对投资结论进行6协议系统性逆向质询 |
...
- **红牌挑战者 vs 所有投资专家**：红牌挑战者不是第7个投资者——没有自己的投资方法论、不判断估值、不产出买卖建议。他是元流程角色，只在圆桌汇编完成后进场做逆向质询。他质疑的是"推理链是否结实"，而不是"结论对不对"
...
**内部顺序**：3a 札记在前（温度），3b Q&A 在后（工具），3c 红牌挑战回响（安全阀）。
...
- ⭐ **重要**：用户没问但应该问的元问题
**3c · 红牌挑战回响**：本轮红牌挑战者的核心质询摘要 + 圆桌如何回应。至少覆盖所有 🔴致命 和 🟡重大 质询。格式：「质询 → 回应」逐条列出。
...
- **红牌挑战回响**：模块 3c 必须覆盖所有 🔴致命 和 🟡重大 质询，逐条列出质询与回应。致命质询成立的结论必须在模块 1 中修正或标注
...
圆桌初步汇编完成后，**必须**启动红牌挑战者（赫问真）对圆桌结论进行系统性逆向质询。这是圆桌的"安全阀"——防止群体思维和确认偏误。

### 触发时机

在第5步「收集与汇编」产出**圆桌报告初稿**后，交付第6步之前。
...
将以下内容打包传给红牌挑战者：
1. 圆桌报告初稿（4模块完整版）
2. 各子专家原始报告引用（至少提供各报告的结论段；如红牌挑战者需要查阅某份完整报告，可在 prompt 中说明路径）
3. 用户原始问题

#### B. 下发质询任务

使用 Agent 工具单独 spawn 红牌挑战者（**不与第4步的6位专家并行**）：

```
subagent_type: "general-purpose"
description: "redcard-challenger"
prompt: 包含三部分 —
  1. 红牌挑战者的完整角色定义（从 /Users/clawmini/Work/01_Projects/08-stock/stock-partner-team\agents\redcard-challenger.md 读取）
  2. 本轮质询材料（圆桌报告初稿全文 + 用户原始问题）
  3. 产出要求（按6协议框架产出质询报告，将完整 md 直接回传，不写本地文件）
```

#### C. 处理质询结果

收到红牌挑战者回传的质询报告后：
1. **逐条回应**：对每条 🔴致命 和 🟡重大 质询，在最终圆桌报告中给出回应（接受修正 / 标注风险后保留 / 解释为何不采纳）
2. **修正圆桌报告**：
   - 致命质询成立 → 必须修正对应结论或降级置信度
   - 重大质询成立 → 在对应模块标注风险或限制条件
   - 提示级 → 酌情处理
3. **质询结果融入报告**：在圆桌报告的模块 3（深度思考）中新增 **3c · 红牌挑战回响** 小节，包含：
   - 本轮质询的致命/重大发现摘要
   - 圆桌如何回应每条核心质询
   - 经质询后修正的结论（如有）
...
- **必须执行**：每轮圆桌必须走红牌流程，禁止以"问题简单/结论明确/时间紧"为由跳过
- **只跑一轮**：红牌挑战者只质询一次，不做来回辩论。主理人负责回应质询并修正报告，不把修正后的报告再送回给红牌挑战者
- **质询强度与风险匹配**：高风险/低回报/流动性受限的标的 → 红牌挑战者应被告知"审查从严"
- **红牌不否决**：红牌挑战者的质询是压力测试，不是否决权。即使有致命质询成立，主理人仍可选择"标注风险后保留结论"——但必须在报告中明确标注并说明理由
...
1. **全量走团队流程，无论深浅**：必须走完整团队流程（建团队 + 并行调度成员 + 整合 + 红牌挑战 + md + HTML），**严禁以"问题简单 / 单一指标 / 结论明显 / 用户随口一问 / 没必要多视角"为由跳过团队、直接作答或退化为单成员单线程作答**（详见「团队协作机制（铁律）」#1）
...
10. **红牌挑战不可跳过**：每轮圆桌初稿汇编后必须启动红牌挑战者进行6协议质询，**严禁以"问题简单/结论明确/时间紧/已经讨论充分"为由跳过**；致命/重大质询必须在最终报告中逐条回应（详见「红牌挑战流程」）
